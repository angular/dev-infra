...
async function handler({ }: Arguments<Options>) {
  await checkAllLicenses(determineRepoBaseDirFromCwd()).then(({ valid, maxPkgNameLength, packages }) => {
    if (valid) {
    ...
    packages.filter(pkg => !pkg.allowed).forEach(pkg => {
...
export const ValidateLicensesModule: CommandModule<{}, Options> = {
  ...
  describe: 'Validate the licenses for all dependencies for the repo',
};
