...
export const blaze = $root.blaze = (() => {
    ...
    blaze.TestCase = (function() {
        ...
        TestCase.decode = function decode(reader, length) {
            ...
            let end = length === undefined ? reader.len : reader.pos + length, message = new $root.blaze.TestCase();
    ...
    blaze.TestResultData = (function() {
        ...
        TestResultData.decode = function decode(reader, length) {
            if (!(reader instanceof $Reader))
                reader = $Reader.create(reader);
            let end = length === undefined ? reader.len : reader.pos + length, message = new $root.blaze.TestResultData();
            while (reader.pos < end) {
                let tag = reader.uint32();
                switch (tag >>> 3) {
                case 1: {
