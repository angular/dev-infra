...
async function handler(options: Arguments<Options>) {
  ...
  if (files.length === 0) {
    console.error(`No files matched the pattern "${options.files}"`);
    process.exit(1);
  }
  ...
  for (const {name, error} of failures) {
    console.log('-------------------------------------');
    console.log(`${name} failed to migrate:`);
    console.log(error);
  }
  ...
  if (failures.length > 0) {
    console.log(`${failures.length} file(s) failed. See logs above for more information.`);
  }
...
async function applyPrompt(
  ...
  if (!text) {
    throw new Error(`No response from the API. Response: ` + JSON.stringify(response));
  }
