...
export function rewriteLinks(aioLink: string): string {
  const shouldRewriteDomain = aioLink.startsWith(AIO_URL);

  const link = shouldRewriteDomain ? aioLink.replace(`${AIO_URL}`, ``) : aioLink;
  const linkWithoutFragment = normalizePath(link).split('#')[0];

  return linksMap.get(linkWithoutFragment) ?? linkWithoutFragment;
}
