...
import worker_protocol from './worker.js';
...
function createFilesystemTree(root, inputs) {
  ...
  function remove(p) {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (node.current[Type] == TYPE.SYMLINK) {
        debug(
          `WEIRD_BAZEL_6_BEHAVIOR: removing ${p} starting from the symlink parent since it's a node with a parent that is a symlink.`,
        );
  ...
  function update(p) {
    ...
    for (const segment of segments) {
      ...
      if (!node[segment]) {
        debug(
          `WEIRD_BAZEL_6_BEHAVIOR: can't walk down the path ${p} from ${currentp} inside ${segment}`,
        );
...
function isExternalLib(path) {
  return path.includes('typescript@') && path.includes('node_modules/typescript/lib');
}
...
function createProgram(args, inputs, output, exit) {
  ...
  const executingfilepath = path.relative(bin, ts.sys.getExecutingFilePath()); // /bazel-bin/<opt-cfg>/bin/node_modules/tsc/...
  ...
  const host = ts.createCompilerHostWorker(cmd.options, true, sys);
  ...
  for (const key in cmd.options) {
    compilerOptions[key] = cmd.options[key];
  }

  console.error(raw, args, cmd);

  refreshProgram();
  enableStatisticsAndTracing();
  applySyntheticOutPaths();

  // early return to prevent from declaring more variables accidentally.
  return {
    getProgram: () => lastProgram?.getTsProgram(),
    emit: (cancellationToken) => lastProgram.emit({cancellationToken, forceEmit: true}),
    applyArgs,
    setOutput,
    formatDiagnostics,
    flushWatchEvents,
    invalidate,
    postRun,
    printFSTree: filesystem.printTree,
  };
  ...
  function enqueueAdditionalWatchEventsForSymlinks() {
    for (const symlink of watchEventsForSymlinks) {
      const expandedInputs = filesystem.readDirectory(
        symlink,
        undefined,
        undefined,
        undefined,
  ...
  function applySyntheticOutPaths() {
    compilerOptions.outDir = path.join(SYNTHETIC_OUTDIR, compilerOptions.outDir);

    if (compilerOptions.declarationDir) {
      compilerOptions.declarationDir = path.join(SYNTHETIC_OUTDIR, compilerOptions.declarationDir);
    }
    ...
    if (compilerOptions.sourceMap || compilerOptions.declarationMap) {
      compilerOptions.mapRoot = path.join(SYNTHETIC_OUTDIR, compilerOptions.outDir);
    }
  }
  ...
  function refreshProgram() {
    const cfg = ngtsc.readConfiguration(cmd.options.project);

    cfg.rootNames = cfg.rootNames.filter((f) => {
      console.error(f);
      f = path.relative(execroot, f);
      console.error(f, filesystem.fileExists(f));
      return filesystem.fileExists(f);
    });

    console.error(cfg.rootNames);

    lastProgram = ngtsc.createProgram({
      host,
      options: cfg.options,
  ...
  function readFile(filePath, encoding) {
    ...
    if (!filesystem.fileExists(filePath) && !isExternalLib(filePath)) {
  ...
  function watchFile(filePath, callback, _) {
    ...
    filePath = path.resolve(sys.getCurrentDirectory(), filePath);
    const close = filesystem.watchFile(filePath, (input, kind) =>
      watchEventQueue.push([callback, path.join('/', input), kind]),
    );
    return {close};
  }
}
...
async function emit(request) {
  ...
  timingStart('emit');
  const result = worker.emit(cancellationToken);
  timingEnd('emit');
  ...
  const diagnostics = ts
    .getPreEmitDiagnostics(worker.getProgram(), undefined, cancellationToken)
    .concat(result?.diagnostics);
  timingEnd('diagnostics');

  const succeded =
    !result.emitSkipped && result?.diagnostics.length === 0 && diagnostics.length === 0;
  ...
  if (ts.performance && ts.performance.isEnabled()) {
    ts.performance.forEachMeasure((name, duration) =>
      request.output.write(`${name} time: ${duration}\n`),
    );
  }
...
if (isMain && worker_protocol.isPersistentWorker(process.argv)) {
  ...
  await execute(ts.sys, ts.noop, args);
}
