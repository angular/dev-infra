...
async function commentOnPr(client, prNumber, body) {
 ...
 * access.
 */
async function deleteComment(client, comment_id) {
...
async function getRequestComment(client, prNumber, body) {
...
async function run() {
    ...
    if (github_1.eventName !== 'pull_request') {
        console.log('not the right event type', github_1.eventName);
        return;
    }
    // The event payload for the Pull Request event.
    const pullRequestPayload = github_1.payload;
    ...
    let client = new rest({ auth: core_5('github-token') });
